...
Detta sparar både tid och pengar vid den första projektkonfigurationen eller vid utvidgning av befintliga AEM-skärmar till nya platser.
...
* **sökväg {string}** Sökvägen till platsen som ska importeras, där sökvägen är platsmappen för projektet (d.v.s. */foo* importeras till */content/screens/&lt;projekt>/locations/foo*)

* **mall {string}** Den mall som ska användas för den nya platsen är för närvarande det enda tillåtna värdet&quot;location&quot;, men detta kommer att användas för alla skärmmallar i framtiden (&quot;display&quot;,&quot;sequencechannel, o.s.v.)
* **[./*]{string}**Valfri egenskap som ska anges på platsen (d.v.s. ./jcr:title, ./jcr:description, ./foo, ./bar). Den aktuella versionen tillåter för närvarande ingen filtrering

>[!NOTE]
>
>Alla kolumner som inte matchar villkoren ovan ignoreras. Om du t.ex. har en annan kolumn definierad i bladets (CSV/XLS) fil än **sökväg**,**mall**,**titel** och **beskrivning** i filen, kommer dessa fält att ignoreras och **Project Importer** validerar inte dessa ytterligare fält för import av projektet till AEM-skärmsprojektet.

## Använda projektimporteraren {#using-project-importer}

I följande avsnitt beskrivs hur projektimporteraren används i ett AEM-skärmsprojekt.
...
1. Guiden **Importera** öppnas. Välj den fil som du har för ditt projekt med platser eller välj den fil (***minimum-file.xls***) som du hämtade under *Krav* .
